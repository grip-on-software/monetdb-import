...
public class EnvironmentDb extends BaseDb implements AutoCloseable {
    ...
    public EnvironmentDb() {
        insertStmt = new BatchedStatement("insert into gros.source_environment(project_id,source_type,url) values (?,?,?)");
    }
     ...
     */
    public void insert_source(int project, String type, String url) throws SQLException, PropertyVetoException {
    ...
    private void getCheckStmt() throws SQLException, PropertyVetoException {
        if (checkStmt == null) {
            ...
            checkStmt = con.prepareStatement("select source_type from gros.source_environment where project_id = ? and url = ?");
        }
    }
     ...
     * @throws PropertyVetoException If the database connection cannot be configured
     */
    public boolean check_source(int project, String url) throws SQLException, PropertyVetoException {
        getCheckStmt();
        
        checkStmt.setInt(1, project);
        checkStmt.setString(2, url);
